...
// 「欠费 / 密钥失效」判定：这类不是瞬时抖动，重试同一个 key 只会一直失败——应立刻切换到备用 key。
// 覆盖两类信号：① 鉴权/付费类 HTTP 状态（401 未授权、402 需付费、403 禁止）；
// ② 响应体/流内错误里的欠费关键字（各家代理措辞不一，尽量宽松匹配中英文）。status 传 0 表示只按文本判。
function isBillingError(status, body) {
  if (status === 401 || status === 402 || status === 403) return true
  return /insufficient|balance|quota|credit|billing|payment|expired|欠费|余额|额度|未充值|已过期|无可用|配额/i.test(String(body || ''))
}
...
// 用单个 api key 完成一次「请求（含瞬时重试）」。返回三种结果：
//   { text }              —— 成功
//   { billing: true, error } —— 该 key 欠费/失效，外层应切换到下一个 key
//   { error }             —— 其它确定性失败 / 预算耗尽，外层直接返回，不再换 key
// t0 是全局起点，多个 key 共享同一 58s 预算——欠费属「秒失败」，切 key 后备用 key 仍有充足预算。
async function runWithKey(apiKey, keyLabel, base, usedModel, payload, t0) {
  const headers = {
    'x-api-key': apiKey,
  ...
  while (true) {
    ...
    try {
      ...
      // 欠费/密钥失效：不重试同一个 key，交外层切换。响应体一并参与判定（有些代理欠费仍回 200 带 JSON 错误）。
      if (isBillingError(resp.status, resp.body)) {
        return { billing: true, error: `AI 服务返回 HTTP ${resp.status}（${keyLabel} 欠费/失效，model=${usedModel}）：${resp.body.slice(0, 200)}` }
      }
      ...
      if (sseErr) {
        // 流内错误：欠费类交外层切 key；过载/限流视为可重试；其余（如安全拦截）如实返回。
        if (isBillingError(0, sseErr)) {
          return { billing: true, error: `AI 流式返回错误（${keyLabel} 欠费/失效，model=${usedModel}）：${sseErr}` }
        }
...
exports.main = async (event) => {
  ...
  // 主 key 在前、备用 key 在后；过滤未配置的空值。任一 key 欠费/失效即自动切下一个。
  ...
  const t0 = Date.now()
  console.log(`[aiProxy] 开始 v=stream+cap20s+multikey model=${usedModel} keys=${keys.length} maxTokens=${maxTokens} promptLen=${prompt.length}`)
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    const res = await runWithKey(keys[i], keyLabel, base, usedModel, payload, t0)
    if (res.text) return { text: res.text }
    lastErr = res.error
    // 仅在「欠费/失效」且「还有下一个 key」且「预算尚够再跑一次」时切换；其它错误如实返回，不浪费备用 key。
    const hasNext = i < keys.length - 1
    const remainingNow = TOTAL_BUDGET - (Date.now() - t0)
    if (res.billing && hasNext && remainingNow > MIN_RETRY_BUDGET) {
      console.warn(`[aiProxy] ${keyLabel} 欠费/失效，切换到 key#${i + 2}（剩余预算 ${remainingNow}ms）：${res.error}`)
      continue
    }
