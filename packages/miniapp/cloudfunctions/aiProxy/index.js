...
// 「欠费 / 密钥失效」判定：这类不是瞬时抖动，重试同一个 key 只会一直失败——应立刻换备用 key。
// 只认状态码，不匹配响应体文本：402 是 gaccode 实测的欠费码，401 是密钥失效。
// 403 特意不算——它可能是内容策略/权限问题，换 key 同样失败，只会白白烧掉备用账号一次请求。
function isBillingError(status) { return status === 401 || status === 402 }
...
// 用单个 api key 跑完一次「请求 + 瞬时重试」。三种返回：
//   { text }                 成功
//   { billing: true, error } 该 key 欠费/失效，外层应换下一个 key
//   { error }                其它确定性失败或预算耗尽，外层直接返回，别浪费备用 key
// post 由调用方传入（生产传 postJson，测试传假实现）——http 层可替换，切换逻辑才测得动。
// t0 是全局起点：多个 key 共享同一份 58s 预算。欠费属「秒失败」，切过去时备用 key 仍有充足预算。
async function runWithKey(post, apiKey, keyLabel, base, usedModel, payload, t0) {
  const headers = {
    'x-api-key': apiKey,
  ...
  while (true) {
    ...
    try {
      const resp = await post(base + '/v1/messages', headers, payload, attemptTimeout(remaining))
      // 欠费/密钥失效：重试同一个 key 是白费，交外层换下一个。
      if (isBillingError(resp.status)) {
        return {
          billing: true,
          error: `AI 服务返回 HTTP ${resp.status}（${keyLabel} 欠费或密钥失效，model=${usedModel}）：${resp.body.slice(0, 200)}`,
        }
      }
      ...
      console.log(`[aiProxy] AI 返回 status=${resp.status} 耗时=${cost}ms model=${usedModel} ${keyLabel} 尝试=${attempt}`)
      ...
      if (!retriable || remainingNow - wait < MIN_RETRY_BUDGET) {
        const cost = Date.now() - t0
        console.error(`[aiProxy] 失败 耗时=${cost}ms model=${usedModel} ${keyLabel} 尝试=${attempt}次 err=${e.message}`)
        return { error: `云函数调用 AI 失败（model=${usedModel}，耗时${cost}ms，尝试${attempt}次）：${e.message}` }
      }
      console.warn(`[aiProxy] 瞬时错误第 ${attempt} 次，退避 ${wait}ms 后重试（${keyLabel}，剩余预算 ${remainingNow}ms）err=${e.message}`)
      await sleep(wait)
    }
  }
}

// 依次拿每个 key 去跑，只有「欠费/失效」才换下一个：其它错误换 key 同样失败，换了纯属浪费。
// 日志只打 key#N 序号，绝不打印 key 明文。
async function runWithKeys(post, keys, base, usedModel, payload, t0) {
  let lastErr = ''
  for (let i = 0; i < keys.length; i++) {
    const keyLabel = `key#${i + 1}`
    const res = await runWithKey(post, keys[i], keyLabel, base, usedModel, payload, t0)
    if (res.text) return { text: res.text }
    lastErr = res.error
    const hasNext = i < keys.length - 1
    const remainingNow = TOTAL_BUDGET - (Date.now() - t0)
    // 预算不足时不再换：切过去也跑不完一次请求，只会拖到被云平台强杀。
    if (res.billing && hasNext && remainingNow > MIN_RETRY_BUDGET) {
      console.warn(`[aiProxy] ${keyLabel} 欠费或密钥失效，切换到 key#${i + 2}（剩余预算 ${remainingNow}ms）`)
      continue
    }
    return { error: res.error }
  }
  return { error: lastErr || 'AI 调用失败' }
}
...
exports.main = async (event) => {
  ...
  // 主 key 在前、备用 key 在后；未配置的自动跳过，所以不配 KEY2 就是原来的单 key 行为。
  const keys = [process.env.GACCODE_API_KEY, process.env.GACCODE_API_KEY2].filter(Boolean)
  if (!keys.length) return { error: '未配置 GACCODE_API_KEY' }
  const t0 = Date.now()
  console.log(`[aiProxy] 开始 v=stream+cap20s+failover model=${usedModel} keys=${keys.length} maxTokens=${maxTokens} promptLen=${prompt.length}`)
  ...
  return runWithKeys(postJson, keys, base, usedModel, payload, t0)
}
