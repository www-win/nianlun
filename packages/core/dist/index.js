          ...
          return {
            ...
            _compute: function() {
              ...
              for (i = 0; i < 15; i++) {
                ...
                this._p.months.push(LunarMonth._(y, mc, dayCounts[i], hs[i] + Solar3.J2000, index));
                index++;
              }
              return this;
            }
...
var require_lunar_javascript = __commonJS({
  "../../node_modules/.pnpm/lunar-javascript@1.7.7/node_modules/lunar-javascript/index.js"(exports, module) {
    "use strict";
    var { Solar: Solar3, Lunar, Foto, Tao, NineStar, EightChar, SolarWeek, SolarMonth, SolarSeason, SolarHalfYear, SolarYear, LunarMonth, LunarYear, LunarTime, ShouXingUtil, SolarUtil, LunarUtil, FotoUtil, TaoUtil, HolidayUtil, NineStarUtil, I18n } = require_lunar();
    module.exports = {
      Solar: Solar3,
      Lunar,
      Foto,
...
function buildBaziChart(birth) {
  const { year, month, day, hour } = birth;
  const solar = hour != null ? import_lunar_javascript.Solar.fromYmdHms(year, month, day, hour, 0, 0) : import_lunar_javascript.Solar.fromYmd(year, month, day);
...
// src/astrology/fortune.ts
var import_lunar_javascript2 = __toESM(require_lunar_javascript(), 1);
var SHENG = { \u6728: "\u706B", \u706B: "\u571F", \u571F: "\u91D1", \u91D1: "\u6C34", \u6C34: "\u6728" };
var KE = { \u6728: "\u571F", \u571F: "\u6C34", \u6C34: "\u706B", \u706B: "\u91D1", \u91D1: "\u6728" };
function wuxingRelation(base, other) {
...
function getDayFortune(date, chart) {
  const lunar = import_lunar_javascript2.Solar.fromYmd(date.year, date.month, date.day).getLunar();
...
function isBranchClash(a, b) {
  return inPairs(CLASH, a, b);
}
function isBranchHarmony(a, b) {
  return inPairs(SIX_HARMONY, a, b);
}
function yearBranch(chart) {
  return chart.pillars.year.charAt(1);
}
...
function parseAstroReading(text) {
  ...
  const fortune = pickText2(r.fortune);
  if (fortune) out.fortune = fortune;
  const affinity = pickText2(r.affinity);
  if (affinity) out.affinity = affinity;
  const advice = pickText2(r.advice);
  if (advice) out.advice = advice;
  return out;
}
function buildBirthExtractPrompt(friend, samples) {
  const displayName = friend.alias || friend.name;
  const sampleBlock = samples.length ? samples.map((s, i) => `${i + 1}. ${s}`).join("\n") : "\uFF08\u672C\u6B21\u65E0\u53EF\u7528\u804A\u5929\u6837\u672C\uFF09";
  return [
...
function parseBirthInfo(text) {
  ...
  const hour = pickInt(r.hour, 0, 23);
  if (hour !== void 0) out.hour = hour;
