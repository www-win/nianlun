...
每个侧面给**一小段简述**（约 30~60 字，可点出推断依据，不只是一个标签）：

| 侧面 | 输出 |
|---|---|
| 身份/职业 | 一小段简述（如「应为某城商行支行负责人，多次提到放款额度、分行考核」） |
| 家庭状况 | 一小段简述 |
| 感情状态 | 一小段简述 |
| 生活方式 | 一小段简述 |
| **投资偏好** | 一小段总述 + 4 个子维度简述：风险偏好 / 关注品类 / 财富与可投线索 / 决策风格与周期 |
...
每个字段都是一小段简述（约 30~60 字），非单个标签：

```typescript
export interface InvestmentProfile {
  summary?: string      // 一小段总述
  risk?: string         // 风险偏好：保守/稳健/平衡/进取…（附一句依据）
...
- **每个字段要求一小段简述（约 30~60 字，可点出聊天里的依据），不要只给一个标签词。**
...
- 卡片：
  - 前 4 个侧面各一块「标题 + 一小段简述」（简述可多行换行，不截断）。
